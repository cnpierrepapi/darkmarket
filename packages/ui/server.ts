...
// Participant 0 is the only wallet the chain funds. The other four start empty,
// and empty means no NIGHT to register for dust, which means no dust to pay a
// fee with, which means every transaction they try dies on "could not balance
// dust". So they get paid first, before anyone tries to boot.
//
// This used to be a script somebody ran by hand. That worked right up until the
// container was the one starting the chain.
// How much NIGHT each of the other four gets.
//
// This number matters more than it looks. Registering NIGHT for dust costs a
// fee, and that fee has to be generated by the NIGHT being registered, so a
// wallet that is funded too thinly sits there for ten minutes waiting to afford
// its own registration. Wallet 0 clears it instantly because genesis holds
// orders of magnitude more.
//
// So: ask for a lot, and walk down if the payer cannot cover it. A chain we did
// not write the spec for gets to decide what genesis is worth.
const FUND_LADDER = (process.env.DARKMARKET_FUND_EACH
  ? [process.env.DARKMARKET_FUND_EACH]
  : ["100000000000000", "1000000000000", "10000000000", "150000000"]
).map((n) => BigInt(n));

const fundParticipants = async (): Promise<void> => {
  // Addresses first, cheaply. None of these need to sync to tell us where they
  // are, and waiting on four syncs here would double the boot.
  const recipients: string[] = [];
  for (let i = 1; i < PARTICIPANTS; i++) {
    const w = await buildWalletFacade(urls as never, seeds[i], NETWORK as never);
    recipients.push(w.unshieldedAddress);
    try { await (w.wallet as { close?: () => Promise<void> }).close?.(); } catch {}
  }

  say("funding participants from wallet 0");
  const payer = await buildWalletAndWaitForFunds(urls as never, seeds[0], NETWORK as never);

  // unshieldedToken() is { tag, raw }. The transfer wants the raw token type.
  const token = unshieldedToken() as unknown as { tag: string; raw: string };
  const decodeAddr = (bech: string) =>
    MidnightBech32m.parse(bech).decode(UnshieldedAddress as never, NETWORK as never);

  let sent: bigint | undefined;
  let lastErr: unknown;
  for (const amount of FUND_LADDER) {
    ...
    try {
      ...
      sent = amount;
      say(`funded ${outputs.length} participants with ${amount} each, tx ${String(txId).slice(0, 18)}`);
      break;
    } catch (e: unknown) {
      lastErr = e;
      say(`funding ${amount} each did not go through, trying less`);
    }
  }
  if (sent === undefined) throw lastErr ?? new Error("funding failed at every amount");

  try { await (payer.wallet as { close?: () => Promise<void> }).close?.(); } catch {}

  // submitTransaction comes back once the node has taken it, not once it is in
  // a block. Blocks here are about six seconds apart, and a wallet that builds
  // before the money lands sees an empty balance and registers nothing for dust.
  await new Promise((r) => setTimeout(r, Number(process.env.DARKMARKET_FUND_SETTLE_MS ?? 30000)));
};
...
(async () => {
  ...
  try {
    ...
    // Worth carrying on. Wallet 0 can still trade, and if the four were already
    // funded from an earlier run this throws for a reason that does not matter.
    say(`funding failed: ${String((e as Error)?.message ?? e).slice(0, 140)}`);
  }
